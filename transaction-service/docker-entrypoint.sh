...
sed -i "s|^DB_HOST=.*|DB_HOST=${DB_HOST:-transaction-db}|" .env
sed -i "s|^DB_PORT=.*|DB_PORT=${DB_PORT:-5432}|" .env
sed -i "s|^DB_DATABASE=.*|DB_DATABASE=${DB_DATABASE:-transaction_db}|" .env
sed -i "s|^DB_USERNAME=.*|DB_USERNAME=${DB_USERNAME:-zenith}|" .env
sed -i "s|^DB_PASSWORD=.*|DB_PASSWORD=${DB_PASSWORD:-zenith}|" .env
...
if grep -q "^ACCOUNT_SERVICE_URL=" .env; then
  sed -i "s|^ACCOUNT_SERVICE_URL=.*|ACCOUNT_SERVICE_URL=${ACCOUNT_SERVICE_URL:-http://account-service:8000}|" .env
else
  echo "ACCOUNT_SERVICE_URL=${ACCOUNT_SERVICE_URL:-http://account-service:8000}" >> .env
fi
